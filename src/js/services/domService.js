...
export class DomService {
  ...
  showWizardContainer() {
    this.load(true);
    const wizardContainerObj = document.getElementById('wizardContainer');
    if (wizardContainerObj.classList.contains('hide')) {
      wizardContainerObj.classList.remove('hide');
    }
  }
  ...
  updateWizardClasses(stepInt) {
    const step = parseInt(stepInt);
    let decrementalIterator = step - 1;
    let incrementalIterator = step + 1;
    let classStartsWith = 2;
    let availableClassCounter = 0;
    const wizardSteps = $('#wizardContainer').children();
    const id = `wizardStep${step}Content`;
    const element = $(`#${id}`);
    element.attr('class', 'row wizardStep normal');
    while (decrementalIterator > 0) {
      const decrementalId = `wizardStep${decrementalIterator}Content`;
      const decrementalElement = $(`#${decrementalId}`);
      if (decrementalElement) {
        decrementalElement.attr('class', `row wizardStep size-shrink_${classStartsWith}`);
      }
      classStartsWith -= 1;
      decrementalIterator -= 1;
      availableClassCounter += 1;
    }
    classStartsWith = 2;
    while (incrementalIterator <= (wizardSteps.length / 2)) { // divide by two to account for line divs added after each wizard step
      const incrementalId = `wizardStep${incrementalIterator}Content`;
      const incrementalElement = $(`#${incrementalId}`);
      if (incrementalElement) {
        incrementalElement.attr('class', `row wizardStep size-grow_${classStartsWith}`);
      }
      classStartsWith -= 1;
      incrementalIterator += 1;
      availableClassCounter += 1;
    }
    this.enableForm(id);
  }
